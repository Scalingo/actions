...
- Checks:
  ...
  - `pact`: execute Pact verification. The default value is `auto`. With this value the action tries to automatically detect if the `pact:verify` rake task exists. Other possible options are `true` and `false`.
- Specs:
